...
void addToTable(int decoder_id, int device_id, int device_id_tx, int stream_type, bool active) {
    LOGV("addToTable (dec_id %d, dev_rx %d, dev_tx %d, type %d, active %d)", decoder_id, device_id, device_id_tx, stream_type, active);
...
static status_t updateDeviceInfo(uint32_t rx_device, uint32_t tx_device,
    ...
    while (temp_head->next != NULL) {
        ...
        switch (temp_ptr->stream_type) {
            ...
            case LPA_DECODE:
            case FM_RADIO:
                LOGV("The node type is %d and cur device %d new device %d ", temp_ptr->stream_type, temp_ptr->dev_id, rx_device);
    ...
    mEffectEnabled(false)
{
    ...
    LOGD("got device_count %d", dev_cnt);
    if (dev_cnt <= 0) {
